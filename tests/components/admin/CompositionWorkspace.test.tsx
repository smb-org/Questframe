...
describe("Kompositions-Workspace", () => {
  it("zeigt Bühne und beide angedockten Rails und rechnet Drag unabhängig vom Zoom", async () => {
    ...
    const drag = async (width: number, height: number, start: { x: number; y: number }, move: { x: number; y: number }) => {
      const { container } = render(<AdminWorkspace api={createCompositionApi(snapshot)} initialBootstrap={bootstrap()} workspace="composition" />);

      expect(screen.getByRole("link", { name: "HUD" })).toBeInTheDocument();
      ...
      await waitFor(() => expect(screen.getAllByLabelText("X")[1]).toHaveValue(320));
      const placement = { x: Number((screen.getAllByLabelText("X")[1] as HTMLInputElement).value), y: Number((screen.getAllByLabelText("Y")[1] as HTMLInputElement).value) };
      cleanup();
      return placement;
    };
  ...
  it("ändert das Placement bei einem Klick ohne Bewegung nicht", async () => {
    const { container } = render(<AdminWorkspace api={createCompositionApi(challengeSnapshot())} initialBootstrap={bootstrap()} workspace="composition" />);
    const log = await screen.findByRole("region", { name: "Challenge-Log verschieben, Pfeiltasten" });
    const canvas = container.querySelector(".preview-canvas");
    if (canvas === null) throw new Error("Preview-Bühne fehlt.");
    ...
    expect(screen.getAllByLabelText("X")[1]).toHaveValue(300);
    expect(screen.getAllByLabelText("Y")[1]).toHaveValue(8);
  });
  ...
  it("verschiebt beim Drag aus der Modulmitte um das Cursor-Delta", async () => {
    ...
    await waitFor(() => expect(screen.getAllByLabelText("X")[1]).toHaveValue(320));
    expect(screen.getAllByLabelText("Y")[1]).toHaveValue(18);
  });
  ...
  it("draggt das HUD in 1:1-Einheiten statt im Challenge-Raster", async () => {
    ...
    await waitFor(() => expect(screen.getAllByLabelText("X")[0]).toHaveValue(112));
    expect(screen.getAllByLabelText("Y")[0]).toHaveValue(112);
  });
  ...
  it("ignoriert fremde Pointer und beendet Drag bei pointercancel", async () => {
    ...
    expect(screen.getAllByLabelText("X")[1]).toHaveValue(320);
    expect(screen.getAllByLabelText("Y")[1]).toHaveValue(28);
  });

  it("verschiebt das fokussierte Challenge-Log per Pfeiltasten in Einheiten", async () => {
    const log = await (async () => {
      render(<AdminWorkspace api={createCompositionApi(challengeSnapshot())} initialBootstrap={bootstrap()} workspace="composition" />);
    ...
    await waitFor(() => expect(screen.getAllByLabelText("X")[1]).toHaveValue(301));
    expect(screen.getAllByLabelText("Y")[1]).toHaveValue(18);
  });
  ...
  it("hält HUD- und Challenge-Save getrennt", async () => {
    ...
    render(<AdminWorkspace api={api} initialBootstrap={bootstrap()} workspace="composition" />);
    await screen.findByRole("region", { name: "Challenge-Log verschieben, Pfeiltasten" });
    const challengeX = screen.getAllByLabelText("X")[1];
    if (challengeX === undefined) throw new Error("Challenge-X-Feld fehlt.");
    fireEvent.change(challengeX, { target: { value: "200" } });
