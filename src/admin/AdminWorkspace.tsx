      ...
      {settingsDraft !== null && <>
        ...
        <fieldset>
          ...
          <div className="settings-grid">
            ...
            <label><span>Wechseltempo</span><select aria-label="Wechseltempo" disabled={disabled || settingsDraft.overflowMode === "cut"} value={settingsDraft.overflowTempo} onChange={(event) => updateSettings({ overflowTempo: event.target.value as ChallengeOverflowTempo })}><option value="slow">langsam</option><option value="medium">mittel</option><option value="fast">schnell</option></select>{settingsDraft.overflowMode === "cut" && <small className="field-help">nur bei Blättern und Durchlaufen</small>}</label>
            <label><span>Erledigte Einträge</span><select aria-label="Erledigte Einträge" disabled={disabled} value={settingsDraft.doneOrder} onChange={(event) => updateSettings({ doneOrder: event.target.value as ChallengeDoneOrder })}><option value="end">ans Ende rücken</option><option value="keep">an ihrem Platz lassen</option></select><small className="field-help">Mit ‚Rest abschneiden‘ verschwinden Erledigte hinter der letzten Zeile.</small></label>
          </div>
        </fieldset>
        <fieldset>
          <legend>Timer</legend>
